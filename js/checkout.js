      ...
      if (canMakePayment) {
        ...
        paymentRequest.on('paymentmethod', async (ev) => {
          ...
          try {
            ...
            ev.complete('fail');
            if (window.toast) {
              window.toast.error('Payment failed. Retry.');
            }
          }
        });
      } else {
  ...
  function updateShippingOptions() {
    ...
    try {
      if (standardLabel) {
        ...
        if (priceEl) priceEl.textContent = isFree ? 'FREE' : '€13.00';
      }
      if (sameDayOption) {
      ...
      switch (paymentIntent.status) {
        ...
        case 'requires_source_action':
          ...
          if (confirmedIntent.status === 'succeeded') {
            ...
            if (window.toast) {
              window.toast.error('Card declined. Try another.', 5000);
            }
            showError('Card declined. Try another.');
            setLoading(false);
          } else {
        ...
        case 'canceled':
          ...
          if (window.toast) {
            window.toast.error('Payment canceled.', 5000);
          }
